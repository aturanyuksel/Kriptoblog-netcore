...
namespace Haber.Web.Controllers
{
    ...
    public class DashboardController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (var file in files)
                {
                    if (file != null && file.ContentLength > 0)
                    {
                        ...
                        var path = Path.Combine(Server.MapPath("~/HaberResimler/Galeri"), resimAdi);
                        file.SaveAs(path);
        ...
        public ActionResult HaberDuzenleme(int? id)
        {
            ...
            else
            {
                ...
                if (result != null)
                {
                    ...
                    var resimListe = haberhelper.HaberResimleriniGetir(haber);
                    ViewBag.yazarlar = yazarhelper.TumYazarlariListele();
                    ViewBag.kategoriler = kategorihelper.TumKategoriler();
                    haber.HaberOkunmaSayisi = (int)ViewBag.haberOkSayi;
                    haber.HaberResimleri = resimListe;
                    return View(haber);
                }
                else
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult HaberDuzenleme(HaberCl haber, int haberID, int kategoriID, int yazarID, string haberEtiketi)
        {
        ...
        public ActionResult KullaniciDuzenleme(HaberUser user, string roleID)
        {
            ...
            huser.PasswordHash = user.PasswordHash;
